...
parser.add_argument("-D", "--xtc", help="Route to the xtc file")
parser.add_argument("-S", "--gro", help="Route to the gro file")
...
if args.xtc and args.gro and args.pdb:
	print "Error: you can not provide a xtc, gro and a pdb file as input"
	exit()

elif args.xtc and args.pdb:
	print "Error: you can not provide a xtc and a pdb file as input"
	exit()
...
elif args.xtc and not args.gro and not args.pdb:
	print "Error: you can not provide only a xtc file as input"
	exit()

elif not args.xtc and args.gro and not args.pdb:
	print "Error: you can not provide only a gro file input"
	exit()
	...
	if args.pdb:
		...
		if args.pdb:
			...
			if args.missing_atoms and not args.gro:
		...
		pdbNames=ef.xtc(args.gro, args.xtc, args.output, args.separation_frame, args.frame_start, args.frame_end, args.nproc, args.selection)		
		print "Done (Date: "+str(time.strftime("%y/%m/%d"))+" Time: "+str(time.strftime("%H:%M:%S"))+")"
	...
	if args.vdw or args.coulomb:
		...
		topology=p2g.convert(pdbNames["frame_0"], args.gro, args.force_field,args.parameter_file, args.output)
		print "Done (Date: "+str(time.strftime("%y/%m/%d"))+" Time: "+str(time.strftime("%H:%M:%S"))+")"
	...
	if args.disulfide:
		if args.gro:
			print "RIP-MD will not determine disulfide bonds for your input because they will be kept through all the simulation due to definition in the gro file... omitting. (Date: "+str(time.strftime("%y/%m/%d"))+" Time: "+str(time.strftime("%H:%M:%S"))+")"
		else:
	...
	if args.cation_pi:
		...
		cp.parse(pdbNames,args.gro, args.output, args.nproc, Nodes, float(args.cation_pi_distance), args.cp_angle1, args.cp_angle2, args.cp_angle3, args.cp_angle4)
		print "Done (Date: "+str(time.strftime("%y/%m/%d"))+" Time: "+str(time.strftime("%H:%M:%S"))+")"
